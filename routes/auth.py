    ...
    if request.method == 'POST':
        ...
        if cpf_limpo == "00000000000":
            ...
            if master and master.check_password(senha):
                login_user(master)
                return redirect(url_for('dashboard.index'))  # Master vai para o painel administrativo geral
            else:
        ...
        if senha_valida:
            ...
            # Redirecionamento correto e segregado por tipo de usuário
            if user.tipo == "MASTER":
                return redirect(url_for('dashboard.index'))
            elif user.tipo == "LIDER":
                return redirect(url_for('leader.dashboard'))
            else:
